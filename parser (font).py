...
def scan(path):
    ...
    Path(root+"-parsed"+path).mkdir(parents=True, exist_ok=True)
    for i in range(0,img.size[1],16):
        for j in range(0,img.size[0],16):
            tile = img_array[i:i+16, j:j+16]
            tile = np.array([[ite(tuple(j)==(0,0,0,255),(0,0,0,255),(0,0,0,0)) for j in i] for i in tile],dtype=np.uint8)
            x = Image.fromarray(tile)
            x.save(root+"-parsed"+path+"/tile"+str(int(i/16)*int(img.size[0]/16)+int(j/16))+".png")
...
def scangif(path):
    ...
    for y in range(0, height, 16):
        for x in range(0, width, 16):
            ...
            out_path = (
                root+"-parsed"+path+"/tile"+str(int(y/16)*int(width/16)+int(x/16))+".gif"
            )
...
def scandir(path):
    ...
    parsed_dir = Path(root+"-parsed"+path)
